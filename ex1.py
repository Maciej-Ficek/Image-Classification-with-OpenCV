...
def load_grayed_images(path):
    ...
    """
    #loading one image to get its shapes
    folder_path = Path(path);
    filepath = folder_path / 'img001.jpeg';
    get_shapes = np.array(cv2.imread(str(filepath)));

    images = np.zeros((600, get_shapes.shape[0], get_shapes.shape[1]));
    for i in range(1, 601, 1):
        filepath = folder_path / f"img{i:03d}.jpeg";
        images[i - 1, :, :] = cv2.cvtColor(np.array(cv2.imread(str(filepath))), cv2.COLOR_BGR2GRAY);
    return images;
